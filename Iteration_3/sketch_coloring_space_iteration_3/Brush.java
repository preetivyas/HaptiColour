...
public class Brush {
  ...
  public void paint(PGraphics layer, float x, float y, PImage brush, PImage brush1) {
    //layer.ellipse(x, y, scaleFactor, scaleFactor);


    //PV added this for brush tooltips one, might need to move it somewhere else

    //brush 1: pulsing ellipse
    //if (true) {
    //angle += 5;
    //double val = cos(Math.toRadians(angle)) * 12.0;
    //for (int a = 0; a < 360; a += 75) {
    //  double xoff = cos(Math.toRadians(a)) * val;
    //  double yoff = sin(Math.toRadians(a)) * val;
    //  //layer.fill(0);
    ...
    //issue: might leak out of wall or give a padding experience
    //float distX;
    //float distY;
    //float avgdist;
    //distX = abs(x - prevX);
